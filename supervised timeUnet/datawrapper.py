...
def extract_ROIs(movie, train_size, intensity_threshold, timesteps, timeinterval, overlapratio = 0.5, normalization = True, pad = None):
    ...
    while x_bl < movie_shape[1]:
        ...
        while y_bl < movie_shape[2]:
            ...
            if np.mean(ROI[0,:,:]) > intensity_threshold:
                ...
                ROIs.append(ROI) # storing temporally augmented timelapse took a lot of space, do that in generating keras sequence
...
class Imagesequence_generator(ks.utils.Sequence):
    ...
    """
    def __init__(self, data, batch_size, timestep, timeinterval, signallevel, gaussnoise, offset):
        ...
        self.dataset_size = 0
        for interval in timeinterval:
            self.dataset_size = self.dataset_size + ((self.data.shape[1]) - (self.timestep - 1) * interval)
        ...
        print("{} timelapse short sequence will be generated!".format(self.dataset_size))
    ...
    def __getitem__(self, idx):
        ...
        def addnoise(data, signal, offset):
            ...
            data_noise = np.random.poisson(data * signal) + np.random.normal(0,gn, data.shape)
        ...
        batch_index = [self.dataset_index[i] for i in range(idx * self.batch_size, min(self.dataset_size, (idx + 1) * self.batch_size))]
        for index in batch_index:
            ...
            frame = range(frame_index,(frame_index + self.timestep * self.timeinterval[interval_index]),self.timeinterval[interval_index])
            ...
            if self.mode != 'n2s':
                target_image = np.expand_dims(target_image, axis=len(target_image.shape))
...
def normalize(x, pmin=0.2, pmax=99.8, axis=None, clip=False, eps=1e-20, dtype=np.float32):
...
def make_sequence(file_path, validation_ratio, batch_size, timestep, timeinterval, signallevel, gaussnoise, offset):
    try:
    ...
    return Imagesequence_generator(data[0:sample_size - validation_size], batch_size, timestep, timeinterval,
                                   signallevel, gaussnoise, offset, training_mode, masker), \
           Imagesequence_generator(data[-validation_size:], batch_size, timestep, timeinterval,
                                   signallevel, gaussnoise, offset, training_mode, masker)

if __name__ == "__main__":
    file_path = os.path.dirname(os.path.realpath(__file__)) + '/Experimental_data/training/'
    ...
    gaussnoise = [5]
    offset = 100
    seq = Imagesequence_generator(data['highSNR'], batch_size, timestep, timeinterval, signallevel, gaussnoise, offset)
    input, target = seq.__getitem__(3)
    index = 0
    if True:
        for i in range(len(input)):
            tiff.imwrite(file_path + 'ROI_{}_input.tif'.format(index), np.squeeze(input[i]), dtype='float32')
            tiff.imwrite(file_path + 'ROI_{}_target.tif'.format(index), np.squeeze(target[i]), dtype='float32')
